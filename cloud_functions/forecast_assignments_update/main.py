...
def forecast_assignments_to_bigquery_update(data: dict, context:dict=None):
    ...
    df['billable'] = df.apply(lambda row: check_billable(row, billable_projects), axis=1)
    
    holidays_df = get_holidays(config, httpx_client)
    
    final_df = pd.concat([change_assignments_columns(df, people_df, projects_df, clients_df).reset_index(drop=True), holidays_df.reset_index(drop=True)])
...
def change_holidays_columns(df):
    ...
    return df.drop(['policyTypeDisplayName', 'policyType',
       ...
       'type',  'status', 
    ], axis=1).rename(columns={'endDate': 'end_date', 'startDate': 'start_date', 'requestId': 'id','employeeDisplayName': 'name'})
...
def get_holidays(config: dict, client: httpx.Client):
    ...
    df = expand_holidays_rows(pd.DataFrame(json.loads(client.get(url, headers=config['headers']).text)['outs']))
...
def change_assignments_columns(df: pd.DataFrame, people_df: pd.DataFrame, projects_df: pd.DataFrame, clients_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['project_colour'] = df.apply(lambda row: get_project_colour(row, projects_df), axis=1)
    df['client'] = projects_df.apply(lambda row: get_client_name(row, clients_df), axis=1)
    df['allocation_hours'] = df['allocation'] / 3600
    df['allocation_days'] = df['allocation_hours'] / 8
    df['holiday_hours'] = 0
    df['holiday_days'] = 0
    df['entry_type'] = 'time_forecast'
    return df.drop(['notes', 'updated_at', 'updated_by_id', 'person_id',
       'repeated_assignment_set_id', 'active_on_days_off', 'allocation' 
    ], axis=1)
...
def get_assignments(api: forecast.Api):
    ...
    return expand_assignments_rows(pd.DataFrame(get_assignments_data(api, start_timestamp, end_timestamp))).drop(['placeholder_id'], axis=1)
