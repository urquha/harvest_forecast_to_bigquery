...
def test_forecast_assignments_to_bigquery_update(
    mock_get_assignments_data,
    mock_write_to_bigquery,
    mock_get_projects_data,
    mock_assignments_response,
    mock_projects_data,
):
...
def test_get_assignments(
    mock_get_assignments_data, mock_assignments_response, mock_expand_rows_response
):
    mock_get_assignments_data.return_value = mock_assignments_response
    response = get_assignments(forecast.Api, -56, 100)
...
def test_expand_rows(mock_assignments_response, mock_expand_rows_response):
    ...
    response = expand_assignments_rows(df).sort_values("start_date")
    print(response.values.tolist())
    assert (
        response.values.tolist()
        == pd.DataFrame(mock_expand_rows_response).values.tolist()
    )
...
def test_get_dates():
    ...
    assert response == [
        datetime(2022, 7, 15, 0, 0),
        datetime(2022, 7, 18, 0, 0),
        datetime(2022, 7, 19, 0, 0),
        datetime(2022, 7, 20, 0, 0),
        datetime(2022, 7, 21, 0, 0),
        datetime(2022, 7, 22, 0, 0),
        datetime(2022, 7, 25, 0, 0),
        datetime(2022, 7, 26, 0, 0),
        datetime(2022, 7, 27, 0, 0),
        datetime(2022, 7, 28, 0, 0),
        datetime(2022, 7, 29, 0, 0),
...
def test_make_row():
    ...
    response = make_assignments_row(row, date)
...
def test_get_billable_codes(mock_get_projects_data, mock_projects_data):
    mock_get_projects_data.return_value = mock_projects_data
    resp = get_billable_codes(pd.DataFrame(mock_projects_data))
    assert resp == [1]
